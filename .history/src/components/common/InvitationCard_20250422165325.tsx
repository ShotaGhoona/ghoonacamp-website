...
export default function InvitationCard({ number, imageUrl, subtitle, title }: InvitationCardProps) {
  return (
    <div className="w-[calc(100%-20px)] md:w-[300px] relative bg-white rounded-xl shadow-xl p-2 md:p-8 flex flex-col items-center text-center ml-[20px] md:ml-0">
      <div 
      className="absolute top-1/2 left-0 -translate-y-1/2 -translate-x-1/2 md:top-0 md:left-1/2 transform md:-translate-x-1/2 
      bg-[#d5697e] shadow-md text-white rounded-full w-12 h-12 md:w-16 md:h-16 flex items-center justify-center text-xl font-bold">
        {number}
      </div>
